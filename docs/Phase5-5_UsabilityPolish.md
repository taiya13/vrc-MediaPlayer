...
## 5-c. 修正: 曲を変えると 3 秒目から始まる

**原因**: 同期の時刻の基準を「ボタンを押した瞬間」に置いていた。
動画の読み込みには 1〜5 秒かかるので、鳴り始めたときには
「もう 3 秒経っているはず」となり、位置合わせがそこへ seek していた。

**直したこと**: 基準を**実際に鳴り始めた瞬間**に置き直す
(`UdonSyncCoordinator.AnchorWhenPlaybackStarts`)。
置き直すまでは位置合わせを止めるので、頭が飛びません。
読み込み中は Now Playing に「読み込み中…」が出たままになります。
