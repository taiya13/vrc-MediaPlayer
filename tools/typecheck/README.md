...
UI の置き場所は「数を足した結果」で決まります。
`mcs` も EditMode テストも、**部品どうしが重なっていること**は教えてくれません。
実機で組み上げて初めて「文字の上にバーが乗っている」と分かります。

Phase8-2 で帯を組み直したとき、この検査を書いた直後に
**6 か所の重なりが一度に見つかりました**。目で追える量ではありません。

### 何を見るか

1. 帯(いま流れている)の中で、部品どうしが重なっていないか
2. 帯からはみ出していないか
3. 縦の積み上げ(帯 → 検索 → タブ → 本体)が 1400 × 860 に収まるか
4. 一覧とレールが、足もとのボタンと重ならずに何行入るか
5. 触れるものが **4.5 cm(≒ 35 px)** を割っていないか

### 寸法はどこから来るか

`UdonMediaPanelBuilder.cs` の `const` を**読み取って**います。
このスクリプトに数を書き写すと必ずずれるので、
ビルダー側で名前付きの `const` にしてから参照してください
(`BandHeight` / `BandTitleHeight` / `BandSeekTouch` など)。

読み取れない名前があると、その場で止まります。
**黙って古い数で通すよりよい**という判断です。
...
`panel_layout.py` は寸法を読み取って**計算し直す**だけなので、
ビルダーの書き方しだいで見落とします。こちらは
`UdonMediaPanelBuilder.BuildWallPanel` / `BuildRemotePanel` を
mono で**そのまま実行**し、できあがった全部品の座標を調べます。
...
1. タブごとに、文字どうし・押す所どうしが重なっていないか
   (バーと「使う」用の区画、入力欄と当たり判定、空のときの案内と行 —— は
   わざと重ねているので除外)
2. 板からはみ出していないか
3. 実行中に入る文字の**長いほう**(「お気に入り  128」「‖  一時停止」
   「夜に駆ける を 再生予定に追加しました。」など)が枠に入るか
4. 「…」シートが**いちばん最後に描かれる**か
   —— uGUI は z(手前 / 奥)ではなく**階層の順**で塗るので、
   z を手前にしても後から作った部品に上書きされます

`UnityEngine.live.cs` は `stubs/UnityEngine.cs` の写しに、
`GameObject` / `Transform` / `Component` / `Mathf` の**本当に動く中身**を入れたものです
(`stubs/` のほうは形だけで、`Mathf.Floor` も 0 を返します)。

### 実績

Phase8-2 の直後に初めて走らせ、`panel_layout.py` では出なかった 5 件を見つけました:
レールのアーティスト名の枠が 30px しか無い / 操作の結果の行が短い /
「…」シートが検索欄・タブ・一覧の下に描かれる(壁パネルとリモコンの両方)。
