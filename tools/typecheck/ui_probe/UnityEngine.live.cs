// ui_probe 専用。stubs/UnityEngine.cs の写しに、
// GameObject / Transform / Component / Mathf / Vector2 の「本当に動く」中身を入れたもの。
// 型検査(typecheck.py)には使いません。
...
namespace UnityEngine
{
    ...
    {
        public Transform transform { get { return gameObject != null ? gameObject.transform : null; } set { } }
        public GameObject gameObject { get; set; }
        public string tag { get; set; }
        public T GetComponent<T>() { return gameObject != null ? gameObject.GetComponent<T>() : default(T); }
        public Component GetComponent(Type t) { return gameObject != null ? gameObject.GetComponent(t) : null; }
        public Component GetComponent(string t) { return null; }
        public bool TryGetComponent<T>(out T c) { c = GetComponent<T>(); return c != null; }
        ...
        public T[] GetComponents<T>() { return gameObject != null ? gameObject.GetComponents<T>() : new T[0]; }
    ...
    public sealed class GameObject : Object
    {
        ...
        public GameObject(string name, params Type[] components)
        {
            ...
            All.Add(this);
            bool rect = false;
            foreach (var t in components) if (t == typeof(RectTransform)) rect = true;
            Transform tr = rect ? new RectTransform() : new Transform();
            tr.gameObject = this; tr.name = name;
            _transform = tr; Components.Add(tr);
            foreach (var t in components) if (t != typeof(RectTransform) && t != typeof(Transform)) AddComponent(t);
        }
        private Transform _transform;
        public Transform transform { get { return _transform; } set { } }
        public string tag { get; set; }
        public int layer { get; set; }
        public bool activeSelf { get { return _active; } }
        public bool activeInHierarchy { get { var p = _transform.parent; return _active && (p == null || p.gameObject.activeInHierarchy); } }
        public Scene scene { get; set; }
        public void SetActive(bool value) { _active = value; }
        public T AddComponent<T>() where T : Component { return (T)AddComponent(typeof(T)); }
        public Component AddComponent(Type t)
        {
            if (t == typeof(RectTransform))
            {
                if (_transform is RectTransform) return _transform;
                var r = new RectTransform(); r.gameObject = this; r.name = name;
                var old = _transform; Components.Remove(old); _transform = r; Components.Insert(0, r);
                r.CopyHierarchyFrom(old);
                return r;
            }
            var c = (Component)Activator.CreateInstance(t);
            c.gameObject = this; c.name = name; Components.Add(c);
            return c;
        }
        public T GetComponent<T>() { foreach (var c in Components) if (c is T) return (T)(object)c; return default(T); }
        public Component GetComponent(Type t) { foreach (var c in Components) if (t.IsInstanceOfType(c)) return c; return null; }
        public bool TryGetComponent<T>(out T c) { c = GetComponent<T>(); return c != null; }
        public T GetComponentInChildren<T>() { return default(T); }
        public T GetComponentInChildren<T>(bool includeInactive) { return default(T); }
        public T GetComponentInParent<T>() { return default(T); }
        public T GetComponentInParent<T>(bool includeInactive) { return default(T); }
        public T[] GetComponents<T>() { var l = new List<T>(); foreach (var c in Components) if (c is T) l.Add((T)(object)c); return l.ToArray(); }
        public Component[] GetComponents(Type t) { return new Component[0]; }
        public T[] GetComponentsInChildren<T>() { return new T[0]; }
        public T[] GetComponentsInChildren<T>(bool includeInactive) { return new T[0]; }
        public T[] GetComponentsInParent<T>() { return new T[0]; }
    ...
    public class Transform : Component, IEnumerable
    {
        ...
        public void CopyHierarchyFrom(Transform old)
        {
            _parent = old._parent;
            if (_parent != null) { int i = _parent.Children.IndexOf(old); if (i >= 0) _parent.Children[i] = this; }
            Children = old.Children; foreach (var c in Children) c._parent = this;
            localPosition = old.localPosition;
        }
        public void SetAsFirstSibling() { if (_parent != null) { _parent.Children.Remove(this); _parent.Children.Insert(0, this); } }
        public void SetAsLastSibling() { if (_parent != null) { _parent.Children.Remove(this); _parent.Children.Add(this); } }
        ...
        public Transform Find(string name) { foreach (var c in Children) if (c.gameObject.name == name) return c; return null; }
        ...
        public void SetSiblingIndex(int index) { if (_parent != null) { _parent.Children.Remove(this); _parent.Children.Insert(Math.Min(index, _parent.Children.Count), this); } }
        public int GetSiblingIndex() { return _parent != null ? _parent.Children.IndexOf(this) : 0; }
