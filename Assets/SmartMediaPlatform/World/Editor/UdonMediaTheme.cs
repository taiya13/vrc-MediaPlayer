...
namespace SmartMediaPlatform.World.EditorTools
{
    ...
    ///
    /// <item><b>文字が主役。</b>Phase8-2 でここを差し替えました。
    ///       <b>このシステムは絵を 1 枚も持ちません。</b>
    ///       サムネイルを焼き込まないと決めた時点で、
    ///       「絵が主役」は<b>守れない規則</b>になりました。
    ///
    ///       代わりを別の絵で埋めることも<b>しません</b>。
    ///       色の四角に頭文字を置く形は、世の中のアプリが
    ///       「写真が未設定です」を表すのに使ってきた形なので、
    ///       どれだけ丁寧に作っても<b>「本当は絵が入るはずの枠」</b>として読まれます。
    ///
    ///       いちばん面積を取るのは<b>曲名</b>です。
    ///       画面でいちばん大きい図形は、Now Playing の帯を横切る
    ///       <b>全幅のシークバー</b> —— 飾りではなく、進み具合そのものです。
    ///       一覧の左端は正方形ではなく<b>番号</b>で、
    ///       鳴っている行だけがそこで<b>動く 3 本の棒</b>に変わります。
    ///       <b>画面で動いている図形は、いつでもこの 2 つだけ</b>です</item>
    ///
